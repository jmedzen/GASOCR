...
# PDF 渲染並發與多執行緒控制 (CPU Core - 2，保底至少 1)
_detected_cores = os.cpu_count() or 4
MAX_RENDER_WORKERS = max(1, int(os.environ.get("MAX_RENDER_WORKERS", _detected_cores - 2)))

# 同時切圖檔案數上限 (預設開放為 CPU 核心數 - 2，保底至少 1)
MAX_CONCURRENT_RENDERS = max(1, int(os.environ.get("MAX_CONCURRENT_RENDERS", _detected_cores - 2)))
